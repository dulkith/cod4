...
class CodxcommandsPlugin(b3.plugin.Plugin):
  ...
  def cmd_ss(self, data, client, cmd=None):
    ...
    """
    ss_site_link = self.config.get('slesports', 'screenshot_website_link')
    if not data:
      ...
      if not target:
        ...
        client.message('Screenshot of %s will be taken soon. Visit ^1' + ss_site_link + ' ^7to view screenshots.' % target.exactName)
