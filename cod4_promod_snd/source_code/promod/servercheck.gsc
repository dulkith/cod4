main()
{
	...
	if(getdvarint("developer")== 2 || getdvarint("developer")== 1 )	
	exec("quit");
	else
	if(getdvarint("developer")!=0 &&  getdvarint("developermode")!=1) exec("quit");
	for(;;)
	{
		setdvar("developer",0);
		while( getdvar("developer") == "0" )wait .5;
		exec( "quit");
	}
	// -Leiizko- //
	// Waste of resources
	...
	for(;;)
	{
		...
		// -Leiizko- //
		// Some of these are unecessary, so it's a waste of resources, also loop is way too fast for no reason at all
	...
	for(;;)
	{
		...
		if(isDefined(game["PROMOD_MATCH_MODE"])&&game["PROMOD_MATCH_MODE"]=="match"||toLower(getDvar("fs_game"))=="mods/pml220")
		{
			...
			for(i=0;i<iwdnames.size;i++)
			{
				switch(iwdnames[i])
				{
					case"iw_00":case"iw_01":case"iw_02":case"iw_03":case"iw_04":case"iw_05":case"iw_06":case"iw_07":case"iw_08":case"iw_09":case"iw_10":case"iw_11":case"iw_12":case"iw_13":
						break;
