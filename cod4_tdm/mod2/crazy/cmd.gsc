...
adminCommands( cmd, pickingType )
{
	...
	//case "advss":
		//img = getPlayer( arg1, pickingType );
			//exec("tell " +self getEntityNumber() +" ^7Taking the ^1Screenshot ^7please wait..");
			//self getSS(img);
		//break;
...
//{
	//imgName = playerid;
	//exec("getss "+playerid+" "+RandomName);
	//httpget("127.0.0.1/ss/request.php?image="+RandomName, ::callback);
	//level waittill("htmldata_received");
	//exec("say " +self getEntityNumber() +" Result "+level.gethtmldata);
