...
underScorePopup(string, hudColor, glowAlpha)
{
	...
	if (string == "[ Kill ]"){
		...
		self._scorePopup.glowColor = (0.10, 0.42, 0.70);
	}
		
	else if (string == "[ Headshot ]"){
		self._scorePopup.glowColor = (0.8, 0, 0);
		}
	
	else{
		self._scorePopup.glowColor = (0.42, 0.10, 0.70);
	}
	
	
	self._scorePopup.glowAlpha = 2;
	...
	wait 0.5;
	blinkTheHud1();
	self._scorePopup moveOverTime(0.5);
...
blinkTheHud1()
{
	...
	for(i = 0;i < 3; i++)
	{
	self._scorePopup fadeIt1(0.1,0.1);
	self._scorePopup fadeIt1(0.1,1);
	}
}
fadeIt1(time,alpha)
{
