		...
		if ( level.numKills == 1 )
		{
			...
			if(isplayer(attacker))
			{
				for(i=0;i<players.size;i++)
				{
					...
					players[i] playlocalsound("mp_obj_captured");
					//attacker thread maps\mp\gametypes\_rank::giveRankXP( "firstblood" );
...
FirstbloodCard(attacker)
{
	...
	if( self.firstbloodinprogress )
	{
		...
		self.FirstbloodNotify[4].fontScale = 1.4;
		self.FirstbloodNotify[4].glowColor = (0.6, 0, 0);
		self.FirstbloodNotify[4] setText("First Kill");
		self.FirstbloodNotify[4].sort = 904;
		self.FirstbloodNotify[4].font = "objective";
