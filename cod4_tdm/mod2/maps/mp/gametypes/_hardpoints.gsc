...
init()
{
	...
	level.hardpointHints["predator_mp"] = "Press [{+actionslot 4}] for PREDATOR";
	level.hardpointHints["orbital_mp"] = "Press [{+actionslot 4}] for ORBITAL SATELLITE";
	level.hardpointHints["carepackage_mp"] = "Press [{+actionslot 4}] for CARE PACKAGE";
	level.hardpointHints["artillery_mp"] = "Press [{+actionslot 4}] for ARTILLERY";
	level.hardpointHints["reaper_mp"] = "Press [{+actionslot 4}] for REAPER";
	level.hardpointHints["strafe_mp"] = "Press [{+actionslot 4}] for STRAFE RUN";
	...
	level.hardpointHints["predator_mp_not_available"] = "PREDATOR not available";
	level.hardpointHints["orbital_mp_not_available"] = "ORBITAL SATELLITE not available";
	level.hardpointHints["carepackage_mp_not_available"] = "CARE PACKAGE not available";
	level.hardpointHints["artillery_mp_not_available"] = "ARTILLERY not available";
	level.hardpointHints["strafe_mp_not_available"] = "STRAFE RUN not available";
	...
	//precacheString( level.hardpointHints["predator_mp"] );	
	//precacheString( level.hardpointHints["orbital_mp"] );	
	//precacheString( level.hardpointHints["carepackage_mp"] );	
	//precacheString( level.hardpointHints["artillery_mp"] );
	//precacheString( level.hardpointHints["strafe_mp"] );
	//precacheString( level.hardpointHints["reaper_mp"] );
...
doPlaneStrike( owner, requiredDeathCount, bombsite, startPoint, endPoint, bombTime, flyTime, direction )
{
	...
	players = level.players;
		for( i = 0; i < players.size; i++ ){
		player = players[ i ];
		player PlaySoundToPlayer( "US_fp2_airstrike", player );
}
...
giveHardpointItemForStreak()
{
	...
	if ( streak < 3 || self.inTrainingArea)
		return;
	...
	if ( streak == 3 )
		...
		self giveHardpoint( "predator_mp", streak );
	else if ( streak == 12 )
		self giveHardpoint( "orbital_mp", streak );	
	else if ( streak == 14 )	
		self giveHardpoint( "carepackage_mp", streak );
	else if ( streak == 16 )
		self giveHardpoint( "artillery_mp", streak );
	else if ( streak == 18 )
		self giveHardpoint( "strafe_mp", streak );				
	else if ( streak == 20 )
...
hardpointNotify( hardpointType, streakVal )
{
	...
	notifyData.titleLabel = &"MP_KILLSTREAK_N";
	notifyData.titleText = streakVal;
	if(hardpointType == "predator_mp" || hardpointType == "strafe_mp" || hardpointType == "carepackage_mp" || hardpointType == "artillery_mp" || hardpointType=="orbital_mp" || hardpointType =="reaper_mp")
		notifyData.notifyText = self getLangString(hardpointType);
	else
...
{
	if ( isDefined( self.pers["hardPointItem"] )  && !self.inTrainingArea)
		...
		switch( currentWeapon )
		{
			...
			case "helicopter_mp":
				...
				{	
					...
					if(self.pers["hardPointItem"] != "predator_mp" && self.pers["hardPointItem"] != "strafe_mp" && self.pers["hardPointItem"] != "carepackage_mp"  && self.pers["hardPointItem"] != "artillery_mp" && self.pers["hardPointItem"] != "orbital_mp" && self.pers["hardPointItem"] != "reaper_mp")
...
triggerHardPoint( hardpointType )
{
	...
	else if ( hardpointType == "radar_mp" && self.pers["hardPointItem"] == "predator_mp")
	{
		if ( isDefined( level.predatorInProgress ) )
		{
			self iPrintBig("PREDATOR_NOT_AVAILABLE");
			return false;
		}
		else if(self GetStance() == "prone") {
			self iPrintBig("PREDATOR_STAND");
			return false;
		}
		else {
			self streakWarning("PREDATOR_INCOMING","FRIENDLY_PREDATOR","ENEMY_PREDATOR");
			self thread duffman\_predator::Predator();
		}
	}
	else if ( hardpointType == "radar_mp" && self.pers["hardPointItem"] == "artillery_mp")
	{
		if(!isDefined(level.artilleryInProgress) && self duffman\_artillery::CanUseArtillery()) {
			self streakWarning("ARTILLERY_INCOMING","FRIENDLY_ARTILLERY","ENEMY_ARTILLERY");
			return true;
		}
		return false;
	}
	else if ( hardpointType == "radar_mp" && self.pers["hardPointItem"] == "strafe_mp")
	{
		if ( isDefined( level.strafeInProgress ) )
		{
			self iPrintBig( "STRAFE_NOT_AVAILABLE" );
			return false;
		}
		else {
			self streakWarning("STRAFE_INCOMING","FRIENDLY_STRAFE","ENEMY_STRAFE");
			self thread duffman\_strafe_run::StrafeRun();
		}
	}
	else if ( hardpointType == "radar_mp" && self.pers["hardPointItem"] == "reaper_mp")
	{
		...
		{
			self crazy\reaper::do_reaper();
			self streakWarning("REAPER_INCOMING","FRIENDLY_REAPER","ENEMY_REAPER");
		}
	}
	else if ( hardpointType == "radar_mp" && self.pers["hardPointItem"] == "carepackage_mp")
	{
		if(self duffman\_carepackage::canCallPackage())
			self streakWarning("PACKAGE_INCOMING","FRIENDLY_PACKAGE","ENEMY_PACKAGE");
		else
			return false;
	}
	else if ( hardpointType == "radar_mp" && self.pers["hardPointItem"] == "orbital_mp")
	{
		self streakWarning("ORBITAL_INCOMING","FRIENDLY_ORBITAL","ENEMY_ORBITAL");
		level thread useOrbital(self);
	}
...
useOrbital(player) {
	players[0] = player;
	if( level.teambased )
		players = getTeamPlayers(player.pers["team"]);
	for(i=0;i<players.size;i++) {
		players[i] setClientDvar("g_compassShowEnemies",1);
		players[i] thread resetOrbital(20);
	}
	wait 20;
	level iPrintSmall("ORBITAL_EXPIRED");
}

resetOrbital(time) {
	level.pausebot = true;
	self notify("new_orbital_satelite");
	self endon("new_orbital_satelite");
	self endon("disconnect");
	wait time;
	self setClientDvar("g_compassShowEnemies",0);
	level.pausebot = undefined;
}
