...
{
	if(sWeapon == "rpg_mp" && eAttacker != self && eAttacker.team != self.team && sMeansOfDeath != "MOD_MELEE"){
		iDamage = 0;
		eAttacker suicide();
		eAttacker iprintlnbold("^1RPG only For The Helicopters");
	}
	
	if(isSubStr( sWeapon, "gl_" ) || isSubStr( sWeapon, "_gl_" )){
		if(sMeansOfDeath == "MOD_GRENADE_SPLASH" || sMeansOfDeath == "MOD_IMPACT"){
			iDamage = 0;
			eAttacker suicide();
			eAttacker iprintlnbold("^1GL is restricted for shoot on players. Use it for HELIs.");	
		}	
	}; 
...
//			}
	...
	if(self.sessionstate != "dead")
	{
		...
		eAttacker thread crazy\randompopup::give(iDamage);
...
//	self maps\mp\gametypes\_weapons::updateWeaponUsageStats();
	...
	//MArena Sadde
		...
		self PlaySoundToPlayer( "generic_pain_american_1", self);
		self PlaySoundToPlayer( "generic_death_american_1", self);
