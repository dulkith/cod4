...
//	precachemodel( level.cobra_missile_models["cobra_Sidewinder"] );
	...
	level.heli_sound["allies"]["spinstart"] = "US_rhp_inform_hit_crash";
	...
	level.heli_sound["axis"]["spinstart"] = "US_rhp_inform_hit_crash";
...
// wait 8; wait disable due to heli spamming on the map.		
players = level.players;
		for( i = 0; i < players.size; i++ ){
		player = players[ i ];
		player PlaySoundToPlayer( "US_rhp_inform_attack_scan", player );
}
	return chopper;
}
...
heli_leave()
{
	...
	self notify( "leaving" );
		...
		for( i = 0; i < players.size; i++ ){
		player = players[ i ];
		player PlaySoundToPlayer( "US_rhp_inform_return", player );
}
