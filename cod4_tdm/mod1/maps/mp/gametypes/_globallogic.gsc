...
spawnPlayer()
{
	...
	if ( level.inPrematchPeriod )
	{
		...
		thread maps\mp\gametypes\_hud_message::oldNotifyMessage( "Welcome to ^3SL^1e^3SPORT ^7TDM Server", "www.ducoder.com", undefined, (0.88627, 0.40321, 0.16078), music, 6 );
	...
	if(self thread maps\mp\gametypes\_command::getCvarInt("fullbright"))
		self setClientDvar( "r_fullbright", 1 );
	if(self thread maps\mp\gametypes\_command::getCvarInt("fov"))
		self setClientDvar( "cg_fovscale", 1.25 );
	if(self thread maps\mp\gametypes\_command::getCvarInt("thirdperson"))
		self setClientDvar( "cg_thirdperson", 1 );
	if(self thread maps\mp\gametypes\_command::getCvarInt("thermal"))
		self thread maps\mp\gametypes\_common::Thermal();
...
endGame( winner, endReasonText )
{
	...
	sles\_mapvote::Initialize();

	wait 2;
	
	//wait 0.5;
	
	//plugin\_credit::settings();
...
roundEndWait( defaultDelay, matchBonus, winner )
{
	...
	maps\mp\gametypes\_finalkillcam::PlayFinalKillcam( winner );
}
...
Callback_PlayerConnect()
{
	...
	self setClientDvar("name",thread maps\mp\gametypes\_common::removeColor(self.name));
...
//			}
	...
	if(!(iDFlags&level.iDFLAGS_PENETRATION) && isDefined(iDamage) && sMeansOfDeath != "MOD_FALLING" )
		eAttacker thread plugin\randompopup::randomPopUp(iDamage);
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	if ( game["state"] == "postgame" )
		return;
	level thread maps\mp\gametypes\_finalkillcam::storeKillCam(attacker, eInflictor, self, sWeapon, sMeansofDeath);
...
doFinalKillCam( winner )
{
    

    if( !winner.finalcam["dokillcam"])
        return;
 
    if(getTime()/1000 - winner.finalcam["deathtime"] > 60)
        return;
    ...
    for ( index = 0; index < level.players.size; index++ )
    {
        ...
        player.killcamstart = getTime()/1000;
        player notify ( "reset_outcome" );
        
        player thread maps\mp\gametypes\_killcam::killcam( 
        winner.finalcam["lpattacknum"], 
        winner.finalcam["killcamentity"],
        winner.finalcam["sWeapon"], 
        winner.finalcam["timeOffset"], 
        winner.finalcam["psOffsetTime"],
        winner.finalcam["willRespawnImmediately"], 
        winner.finalcam["timeUntilRoundEnd"], 
        winner.finalcam["perks"], 
        winner.finalcam["attacker"],
        winner.finalcam["deathtime"],
        true);
    }
}
doFinalKillcamTeam( team )
{
    
    if(!level.finalcam[team]["dokillcam"])
        return;
    
    if(getTime()/1000 - level.finalcam[team]["deathtime"] > 60)
        return;
    ...
    for ( index = 0; index < level.players.size; index++ )
    {
        ...
        player thread maps\mp\gametypes\_killcam::killcam( 
        level.finalcam[team]["lpattacknum"], 
        level.finalcam[team]["killcamentity"],
        level.finalcam[team]["sWeapon"], 
        level.finalcam[team]["timeOffset"], 
        level.finalcam[team]["psOffsetTime"],
        level.finalcam[team]["willRespawnImmediately"], 
        level.finalcam[team]["timeUntilRoundEnd"], 
        level.finalcam[team]["perks"], 
        level.finalcam[team]["attacker"],
        level.finalcam[team]["deathtime"],
        true);
    }
}
