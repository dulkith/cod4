...
init()
{
   ...
   if(getDvar("g_allowfinalkillcam") == "")
       setDvar("g_allowfinalKillcam",1);
	
	level.killcam = maps\mp\gametypes\_tweakables::getTweakableValue( "game", "allowkillcam" );
	level.finalkillcam = getDvarInt("g_allowfinalkillcam" );
...
)
{
	...
	if(attackerNum < 0)
   {
	...
	if(level.showingfinalkillcam)
        predelay = self.killcamstart - deathtime;
   ...
   else
   {
       if ( !isdefined( self.kc_nametext ) )
       {
          ...
          self.kc_nametext.alignY = "middle";
          self.kc_nametext.horzAlign = "center_safearea";
          self.kc_nametext.vertAlign = "top";
          self.kc_nametext.sort = 1; // force to draw after the bars
          self.kc_nametext.font = "objective";
          self.kc_nametext.foreground = true;
          
          if ( level.splitscreen )
          {
             self.kc_nametext.y = 34;
             self.kc_nametext.fontscale = 1.6;
          }
          else
          {
             self.kc_nametext.y = 90;
             self.kc_nametext.fontscale = 1.5;
          }
       }
