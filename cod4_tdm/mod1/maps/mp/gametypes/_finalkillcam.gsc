...
storeKillCam(attacker, eInflictor, victim, sWeapon, sMeansofDeath) {
	if(	isDefined(attacker) && isDefined(victim) && (isPlayer(attacker) || isDefined(attacker.owner) && isPlayer(attacker.owner)) && isPlayer(victim) && attacker != victim) {
		team = attacker.team;
...
finalkill(attacker)
{
	if(isDefined(attacker))
	{
		...
		level.killcamAttacker.label = &"^1Game Winning Kill : &&1";
