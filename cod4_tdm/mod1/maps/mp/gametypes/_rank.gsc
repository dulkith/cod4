 ...
 
{
 ...
 
	{
		...
		registerScoreInfo( "kill", 20000 );
 
		registerScoreInfo( "headshot", 20000 );
	...
	registerScoreInfo( "win", 100000 );
 
	registerScoreInfo( "loss", 0.5 );
 
	registerScoreInfo( "tie", 0.75 );
 
	registerScoreInfo( "capture", 10000 );
 
	registerScoreInfo( "defend", 10000 );
...
self endon("death");
	...
	while ( 1 )
	{
		if(isSubStr( self getCurrentWeapon(), "gl_" ))
		{
			...
			{
				self iPrintlnBold("Grenade Launchers aren't allowed on this server");
				self takeAllWeapons();
